...
void Inorder_print(AVLTreeNode *current);
 ...
 if(filename == "stdin"){
        ...
        while(!feof(fp)){
            fscanf(fp,"%*[(]%d%*[,]%d*[)]",&key,&value);
                flag = InsertNode(newTree,key,value);
                if(flag == 0){
                    printf("Error!");
                    return NULL;
                    }
...
void tree_balance(AVLTree *notbalance){
...
int InsertNode(AVLTree *T, int k, int v)
{
	...
	while(1){
        if(current -> key > k || (current->key ==k && current->value > v)){
